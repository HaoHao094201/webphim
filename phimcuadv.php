<?php
    ...
    if (isset($_GET['ID'])) 
    {
        ...
        join dienvien on phim.DienVien = dienvien.ID_DienVien WHERE DienVien=$id limit ".$item_per_page." offset ".$offset."";
        $phim = mysqli_query($conn, $sql);  
        $phimm =  mysqli_query($conn, "SELECT * FROM phim WHERE DienVien=$id" );
    ...
    $sql = "SELECT * FROM dienvien WHERE ID_DienVien = $id";
    $dv = mysqli_query($conn, $sql);
    $tendv = mysqli_fetch_assoc($dv);
?>

<div class="than">
    <div class="left">
        <div class="menu">
            <div class="l"><b style="font-size: 30px; color: #f3ff50fd;">Phim của <?=$tendv['TenDienVien']?></b></div>
        </div>
        <div class="anhĐC">
            <?php
